...
arg_parser = argparse.ArgumentParser(
    description="Options for generating synonyms"
)
...
def generate_synonyms_file(min_similarity: float) -> None:
    ...
    for __, row in df_top_words.iterrows():
        ...
        list_similarity_vs_word: List[
            Tuple[float, str]
        ] = model.get_nearest_neighbors(word_origin)
        ...
        synonyms: List[str] = [word_origin] + [
            entry[1]
            for entry in list_similarity_vs_word
            if entry[0] >= min_similarity
        ]
        if len(synonyms) > 1:
            df_synonyms = df_synonyms.append(
                pd.DataFrame(
                    {COLNAMES.SYNONYMS: ",".join(synonyms)}, index=[0]
                )
            )

    # Generates synonyms.csv
    df_synonyms.to_csv(
        IO_PATHS.OUTPUT_SYNONYMS, header=None, index=False, sep="\t"
    )
